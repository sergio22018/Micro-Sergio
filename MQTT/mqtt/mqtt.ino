...
const long intervalo = 30000; // cada 30s enviamos la lectura
bool ledState = LOW;
...
void loop() {
  ...
  // Enviar la posición del potenciómetro cada 2 segundos
  if (millis() - lastMsg > intervalo) {
    lastMsg = millis();
    int valor_analogico = analogRead(inputPin); // Lee el potenciómetro
    Serial.println("Valor del potenciómetro: " + String(valor_analogico));
